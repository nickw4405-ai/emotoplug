...
async function ebayScrapeDirect(q, condition) {
  ...
  const url = `https://www.ebay.com/sch/i.html?_nkw=${encodeURIComponent(q)}&LH_BIN=1&_sop=15${condStr}&_ipg=5`;
  try {
    ...
    // eBay item links appear as /itm/<numeric-id> or /itm/<title>/<id>
    $('a[href*="/itm/"]').each((_, el) => {
      if (found) return false;
      const href = $(el).attr('href') || '';
      const m = href.match(/\/itm\/(?:[^/?#]+\/)?(\d{10,})/);
      if (m) {
        found = `https://www.ebay.com/itm/${m[1]}`;
        return false;
      }
    });
...
export async function ebayDirect(q, condition = null) {
  ...
  if (!result) result = await ebayScrapeDirect(q, condition);
