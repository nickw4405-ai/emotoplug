...
export async function POST(req) {
  ...
  // Send via Resend if configured — otherwise fall back to showing link in browser (dev only)
  ...
  if (resendKey) {
    await fetch('https://api.resend.com/emails', {
      ...
      body: JSON.stringify({
        from:    'emotoplug <noreply@emotoplug.com>',
        to:      [email],
    ...
    return NextResponse.json({ ok: true }); // don't return resetUrl when email is sent
  }

  // Resend not configured — return link directly (only for dev/fallback)
